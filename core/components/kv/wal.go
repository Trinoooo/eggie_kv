...
func (b *Block) UnMarshal(rawBytes []byte) (int64, error) {
	...
	b.payload = rawBytes[offsetPayload:]
...
*/
func (wal *WriteAheadLog) persistence() {
	defer utils.HandlePanic(func() {
	...
	for range wal.ticker.C {
		utils.WrapLock(&wal.mu, func() {
			tmpFile := consts.BaseDir + "/wal_cycle_log.swap"
			realFile := consts.BaseDir + "/wal_cycle_log"
			if err := exec.Command("cp", realFile, tmpFile).Run(); err != nil {
				log.Fatalln(err)
			}

			fd, err := utils.CheckAndCreateFile(tmpFile, syscall.O_APPEND|syscall.O_CREAT|syscall.O_RDWR, 0660)
			if err != nil {
				log.Fatalln(err)
			}
			...
			for i := 0; i < len(wal.Blocks); i++ {
				...
				if fileSize+writtenByteSize > MaxWalFileSize {
					_, err := fd.Seek(0, 0)
					writtenByteSize = 0
					if err != nil {
						log.Fatalln(err)
...
func (wal *WriteAheadLog) fixData() error {
	// todo
	return nil
}
